 ...
 */
public class CPIMFromHeader implements ICPIMHeader {

    public static final String NAME = "From";

    private final Address address;

    public CPIMFromHeader(Address address) {
    ...
    public static CPIMFromHeader parse(String headerValue) throws IOException {
        try {
            ...
            return new CPIMFromHeader(new Address(result, chunks[0].trim(), null));
        } catch (Exception e) {
    ...
    public String getValue() {
        ...
        if (address.getDisplayName() != null && address.getDisplayName().length() > 0) {
            sb.append(address.getDisplayName()).append(" ");
        }
        sb.append("<").append(address.getUri()).append(">");
        return sb.toString();
    }
